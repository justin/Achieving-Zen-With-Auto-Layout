...
When you decide to define your interface using Auto Layout, you're no longer explicitly setting the frame values for your controls and views. Instead, you're defining relationships between attributes on two views. Those constraints-based relationships are actually a system of linear equations that uses Cassowary to calculate the respective values.

![The Auto Layout Equation](./images/ch03-ss01.png)

When you define a Cassowary constraint-based relationship between two views, you are in fact defining them around the basis on one of the following linear equations.

If you remember your high school algebra, you may remember this equation:

    y = mx + b

In high school, this was called slope-intercept form, but for our purposes we can just think of it as the basis of Cassowary, and thus Auto Layout.
...
*   `NSLayoutAttribute.Baseline` - The baseline of a view's alignment rectangle. A **baseline** is a typographic term that defines the imaginary line that all text rests on. This is mostly useful for things like buttons and labels where you are working with text.
*   `NSLayoutAttribute.FirstBaseline` - New in iOS 8 as well, you can lay out your view on the first baseline it encounters when working with the topmost line of text.
*   `NSLayoutAttribute.LastBaseline` - This is the similar to `.Baseline`, but allows you to lay out your text based based on the first baseline encountered on the bottommost line of text.  Both this and `NSLayoutAttribute.FirstBaseline` will provide a better layout visually when working with text rather than working with `NSLayoutAttribute.Top` or `NSLayoutAttribute.Bottom`, especially when working with dynamic text.
*   `NSLayoutAttribute.NotAnAttribute` - A non-existent attribute. This is useful for when you aren't establishing a relationship between two views, but instead defining an attribute on the single view. For instance, defining a view's height or width.

Another new concept introduced in iOS 8 is the `layoutMargins` property on `UIView` that lets you explicitly define the whitespace that your views use to guide where portions of the interface should be placed.
...
The multiplier is one of the core algebraic aspects of the Auto Layout linear equation that gives it its power. You can use it to, for instance, say that you want the width of view A to be half of the height of view B or similar.
...
One example where I have used a different multiplier is with adjusting the padding between views based on the dynamic text size. For instance, if the user has a larger dynamic type size, I'll use that value as part of the multiplier so that there's more padding between different labels. If they have a smaller text size, I'll then in turn use a smaller amount of padding.
...
Let's look at an example of this tweet button view. Inside is both a `UIImageView` with the home icon and a `UILabel` that says "Home" in English. In our view subclass, we can override the `intrinsicContentSize` method to calculate and return a value as a `CGSize`.
...
The first line defines a `Dictionary` with the attributes our label is laid out using. We can get then get the `CGSize` of that label by passing the dictionary to the `sizeWithAttributes()` method.

Next, we get the `UIImage` size and store it as a `CGSize` value.

The next two lines define the width and height that we are going to use as our intrinsic content size. The `width` is just the sum of the `imageSize` and `labelSize` widths. The `height` takes the largest (max) value of the `imageSize` and `labelSize` heights.
...
When your view is doing its measurement pass to calculate the size of its frames, it will measure it based on the attributes you have passed to your `UILabel` (or `UITextView`, etc) and lay it out based on that.

The one caveat to this is that you shouldn't set an explicit height or width to that label or other text layout view. Doing so opts you out of using the intrinsic content size to size the view. We'll cover this more in Chapter 8.

### Alignment Rectangles

If the intrinsic content size defines the size of the actual content that we are wanting to lay out, the **alignment rectangle** is what the Auto Layout engine actually uses to line up and lay out the actual controls rather than the full frame of your view.

An alignment rect is useful if you're working with a view that has extra ornamentation, such as a badge on a button that you don't necessarily want to take into account when laying out the control.
...
If you need to do something more complex than just insetting the full-frame of a view, you can override `frameForAlignmentRect:` to define an actual CGRect or NSRect depending on which OS you are targeting. UIImage also has a method `imageWithAlignmentRectInsets` that you can override to return a new version of the image adjusted by those insets.

### Priorities

Sometimes, you'll have multiple constraints that can have a different effect on a specific view. For instance, you may have a set of constraints stating that the left and right edges of the view should be a value of 10, while also saying the view should be less than or equal to 300 points wide.

We can use priorities to decide which one takes precedence over the other if we get into a situation where these constraints may conflict.
...
This chapter was heavily focused on jargon and terminology, but the intent was to provide a good introduction and understanding of some of the key concepts that we will be working with going forward in our journey towards achieving Auto Layout Zen.

In the next chapter, we'll continue to build our foundation of understanding by having an in-depth discussion of what actually happens behind the scenes when the Auto Layout engine does its work.
